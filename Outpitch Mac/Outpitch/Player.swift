...
class Player: Register, UIPickerViewDataSource, UIPickerViewDelegate, UITextFieldDelegate {
    ...
    /* We initialize a tab containing all the names that we want to display in the different picker views when the user touche a TextField
     * We have tab for the countries, clubs, sports, and position.
     */
    ...
    /* We create the Pickerviews items, for each textfield, and a toolbar that will be displayed on the pickerview.
     * We also create the activeTextField variable that will contain the textField that the user selected.
     */
    ...
    override func viewDidLoad() {
        ...
        /* We create the toolbar and we set all its properties, like the style, the color and the buttons that it will contain. */
        ...
        // We delegate the pickerviews and set the datasources
    ...
    // They will basically remove the pickerview and the toolbar from the screen.
    ...
    //return the number of rows in each tab depending of the TextField selected.
    ...
    // Function pickerView : if a row is selected, display it in the text field
    ...
    /*
     Same way of process than for Register. We recover value of pseudo insert before in order to insert data (sport/position/nation/club)
     for the good user in database using in php file "Where username = username"
     */
    @IBAction override func BT_REGISTER(sender: UIButton) {
        ...
        if( nation.isEqualToString("") || positionnement.isEqualToString("") || club.isEqualToString("")){
            ...
            NSLog("PostData: %@",post);
            let url = NSURL(string:"http://localhost:8888/Outpitch/iOSplayer/player.php")!
            ...
            if ( urlData != nil ) {
                ...
                if (res.statusCode >= 200 && res.statusCode < 300){
                    ...
                    if(success == 1){
                        /*
                         We put all our variables in objects to refund theme if you don't disconnect yourself to the app.
                         We also use it as static variable used in a lot of window/class in the app.
                         So there is the same variables in Register + Player than in Connection.
                         */
                        ...
                        /*
                         The value Alert is put to 4 if the registration is ok
                         And we use segue 'S_ENDPLAYER' see the end of the code
                         */
                        Alert = 4
    ...
    /*
     If we change window using segue 'S_ENDPLAYER' we give to variable of Alert in Profile class,
     the value of Alert in this class. (4)
     In the other class you can see that if Alert is equal to 4 (if you just register yourself),
     an alertmessage appears as 'Welcome on Outpitch'
     */
