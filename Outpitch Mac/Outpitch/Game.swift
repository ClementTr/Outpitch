...
class Game: UIViewController, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    var teamName = String()
    //We create a tab of 20 numbers that will be used in Pickerviews just like in Player.swift.
    ...
    //We create 2 variables for the dates that will be used for the DatePickers
    ...
    override func viewDidLoad() {
        ...
        // We create a date format with the today's date which will be displayed in the textfield as the default date.
    ...
    /* When the use touche the TextField date, it opens a datePicker DV_DATE */
    ...
    /* Done and cancel functions for the pickers work the same as in Player.swift except that they now include an option for
     When the TextField selected is the Date, it will remove the DatePicker and its toolbar */
    ...
    /* The datePickerValueChanged is called when the date of the DatePicker is modified, it will put the date in the good format for the TextField and
     also the appropriate format for the database which is yyyy-MM-dd and the date with the good format for database is stored in dateGame variable that
    ...
    /*
     Create new game and add 24 hours to actual time, change it as string and put in
     database for Time value in table Team.
     */
    @IBAction func BT_START_VOTE(sender: UIButton) {
        ...
        if( idPlayer == "" || teamName == "" || matchdate.isEqualToString("") || goalsUs.isEqualToString("") || goalsTheme.isEqualToString("") || opponent.isEqualToString("") ){
            ...
            let url = NSURL(string:"http://localhost:8888/Outpitch/iOSgame/game.php")!
