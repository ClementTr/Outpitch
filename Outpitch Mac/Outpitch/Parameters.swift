...
class Parameters: UIViewController, UIImagePickerControllerDelegate, UIPickerViewDelegate, UITextFieldDelegate, UIPickerViewDataSource, UINavigationControllerDelegate {
    ...
    /* ImagePicker is used for changing profile picture in the Parameters screen, it works exactly in the same way as in Profile.swift except that it will be called
     When the user will touche the BT_PP item and not the settings button...  */
    
    var ImagePicker = UIImagePickerController()
    
    /* We initilize tabs that are contained in the pickerViews the pickerViews works in the same way as in the Player.swift file... */
    ...
    /*
     Same access as before, just SQL request in php file is different because it won't be an INSERT but an UPDATE ... SET 
     for change parameters already existing
     */
    ...
    @IBAction func BT_UPDATE(sender: AnyObject) {
        ...
        if( nation.isEqualToString("") || positionnement.isEqualToString("") || club.isEqualToString("") || firstname.isEqualToString("") || lastname.isEqualToString("") || age.isEqualToString("") || pseudo.isEqualToString("") || email.isEqualToString("") || mdp.isEqualToString("") || password_confirmation.isEqualToString("")){
            ...
            let url = NSURL(string:"http://localhost:8888/Outpitch/iOSupdate/update.php")!
